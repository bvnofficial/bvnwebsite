...
import { Waves, TrainFront, Zap, Wifi, Search, RefreshCw, AlertTriangle, MapPin } from "lucide-react";

type EventType = "flood" | "rail" | "power" | "internet";
...
interface ProviderStatus {
  name: string;
  state: "operational" | "reported" | "outage";
  note?: string;
  source_url?: string;
  at?: string;
}
...
const TYPES: EventType[] = ["flood", "rail", "power", "internet"];
...
export default function OutageMap() {
  ...
  useEffect(() => {
    ...
    loadLeaflet().then((L) => {
      ...
      const map = L.map("bvn-outage-map", { zoomControl: true, scrollWheelZoom: false }).setView(
        [12.4, 122.0],
        6
      );
  ...
  useEffect(() => {
    ...
    for (const e of visible) {
      const color = e.severity === "severe" ? "#ef4444" : TYPE_META[e.type].color;
      ...
      marker.bindPopup(
        `<strong>${e.title}</strong><br/><span style="color:#555">${TYPE_META[e.type].label} · ${e.region} · ${e.severity.toUpperCase()}</span><br/><span style="color:#555">${e.source_name}${e.sample ? " · sample" : " · live"} · ${timeAgo(e.started_at)}</span><br/><a href="${e.source_url}" target="_blank" rel="noreferrer">source</a>`
      );
      try {
  ...
  function focus(e: OutageEvent) {
    mapRef.current?.setView([e.lat, e.lng], 11, { animate: true });
  }

  const counts = useMemo(() => {
    const c: Record<EventType, number> = { flood: 0, rail: 0, power: 0, internet: 0 };
    ...
    <div className="overflow-hidden rounded-2xl border border-white/10 bg-white/[0.03]">
      ...
      <div className="flex flex-wrap items-center gap-3 border-b border-white/10 px-4 py-3">
        ...
        {net && (
          <span className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs text-slate-300">
            <span
              ...
              style={{ background: net.status === "normal" ? "#22c55e" : net.status === "degraded" ? "#f59e0b" : "#ef4444" }}
            />
        ...
        <span className="ml-auto inline-flex items-center gap-1 text-xs text-slate-400">
          ...
          {data ? `Updated ${timeAgo(data.generated_at)} · auto-refresh 60s` : "Loading live feeds…"}
        </span>
      </div>
          ...
          {providers.map((p) => {
            ...
            return p.source_url && p.state !== "operational" ? (
              <a
                ...
                rel="noreferrer"
                title={p.note}
                className="inline-flex items-center gap-1.5 rounded-full border border-white/10 bg-white/5 px-2.5 py-1 text-xs hover:border-white/25"
              >
                {inner}
              </a>
            ) : (
      ...
      <div className="flex flex-wrap items-center gap-2 px-4 py-3">
          ...
          return (
            <button
              ...
              className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-xs font-medium transition ${
                on ? "border-white/25 bg-white/10 text-white" : "border-white/10 bg-transparent text-slate-500"
              }`}
            >
        ...
        <button
          ...
          className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-xs font-medium transition ${
            issuesOnly ? "border-red-500/40 bg-red-500/15 text-red-300" : "border-white/10 bg-transparent text-slate-500"
          }`}
        >
        ...
        <div className="relative ml-auto">
          ...
          <input
            ...
            placeholder="Search city or line…"
            className="w-44 rounded-full border border-white/10 bg-white/5 py-1.5 pl-8 pr-3 text-xs text-white placeholder:text-slate-500 outline-none focus:border-white/30"
          />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_340px]">
        <div id="bvn-outage-map" className="h-[380px] w-full bg-slate-200 lg:h-[540px]" />
        <div className="max-h-[540px] overflow-y-auto border-t border-white/10 lg:border-l lg:border-t-0">
          {visible.length === 0 && (
            <div className="p-4 text-sm text-slate-400">
              {loading ? "Loading live feeds…" : "Walang tugmang alerto sa mga filter na ito."}
            </div>
          )}
          {visible
            .slice()
            ...
            .map((e) => (
              ...
              >
                <div className="flex items-center gap-2">
                  <span
                    className="rounded px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wide text-slate-900"
                ...
                <div className="mt-1 text-[11px] text-slate-400">
                  {e.region} ·{" "}
                  <span className={e.severity === "severe" ? "font-semibold text-red-400" : ""}>
      ...
      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 border-t border-white/10 px-4 py-2.5 text-[11px] text-slate-400">
        {TYPES.map((t) => (
          <span key={t} className="inline-flex items-center gap-1.5">
            <span className="inline-block h-2.5 w-2.5 rounded-full" style={{ background: TYPE_META[t].color }} />
            {TYPE_META[t].label}
          </span>
        ))}
        <span className="inline-flex items-center gap-1.5">
          <span className="inline-block h-2.5 w-2.5 rounded-full" style={{ background: "#ef4444" }} /> Severe
        </span>
        <span className="ml-auto">
          Data: NASA POWER, IODA, Meralco, NGCP, Google News. Hindi opisyal na emergency source.
        </span>
      </div>
    </div>
  );
