...
import {
  ...
  TrendingUp, Brain, Boxes, ShieldCheck, TriangleAlert,
  DollarSign, CreditCard, ListChecks, Mail, Megaphone,
} from "lucide-react";
...
const phases: Phase[] = [
  {
    ...
    items: [
      ...
      { label: "Deliver costed, dependency-ordered build plan + fixed quotes for Phases 1 to 4", status: "todo" },
    ],
  },
  {
    code: "Phase 1",
  ...
  },
  {
    ...
    items: [
      ...
      { label: "Ad scripting (Shopify DTC): winning format + avatar research into grounded ad concepts", status: "todo", ref: "#15" },
...
// ── Payment milestones ───────────────────────────────────────
type PayMilestone = { tag: string; Icon: typeof Search; accent: string; title: string; price: string; when: string; desc: string };
const payMilestones: PayMilestone[] = [
  {
    tag: "Start here",
    Icon: Search,
    accent: C.cyan,
    title: "Scope & Feasibility Sprint",
    price: SCOPE_FEE,
    when: "Paid once, to begin",
    desc: "The only fixed number to get moving. You get the costed, dependency-ordered plan, fixed quotes for every phase after it, and the first quick win already live.",
  },
  {
    tag: "Per phase",
    Icon: Rocket,
    accent: C.blue,
    title: "Build phases 1 to 4",
    price: "Fixed quote each",
    when: "Approved and paid per phase",
    desc: "Each engine is a flat price, set at the end of Phase 0. You approve it before it starts and pay on the milestone, so you only ever commit to the next piece in front of you.",
  },
  {
    tag: "Once live",
    Icon: TrendingUp,
    accent: C.green,
    title: "Active build & growth retainer",
    price: RETAINER_FROM,
    when: "Month to month, no lock in",
    desc: "Not passive babysitting. Active weekly work: keeping the live engines healthy, improving what runs, and building new automations on top. Your team runs the day to day, and anything genuinely large gets its own fixed quote so it never eats the retainer.",
  },
];
...
const dayOne: string[] = [
  ...
  "GoHighLevel audit kicked off, a first read on what you're paying for and not using.",
      ...
      <div style={{ maxWidth: 980, margin: "0 auto", padding: "28px 20px 80px" }}>
        ...
        <div style={{ marginTop: 22 }}>
          ...
          <p style={{ color: C.sub, fontSize: 15, maxWidth: 680, margin: 0 }}>
            ...
            stands, what we tackle on day one, how payment works, and the verdicts on the parts you flagged as risky.
            Managed end to end, so you stop being the bottleneck.
          </p>
        </div>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 14, marginTop: 22 }}>
          <div style={{ flex: "1 1 260px", background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, padding: 18 }}>
            ...
            <div style={{ color: C.muted, fontSize: 12, marginTop: 8 }}>
              {done} of {total} items complete · status: <span style={{ color: C.amber }}>Ready to start, awaiting kickoff</span>
            </div>
          </div>
          ...
          <div style={{ flex: "1 1 200px", background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, padding: 18 }}>
            <div style={{ color: C.sub, fontSize: 13, display: "flex", alignItems: "center", gap: 6 }}>
              <DollarSign size={15} /> Commercial model
            </div>
            ...
            <div style={{ color: C.muted, fontSize: 12, marginTop: 6 }}>
              Phases 1 to 4 priced after Phase 0 · active retainer {RETAINER_FROM}, month to month
            </div>
          </div>
        ...
        {/* Payment milestones */}
        <Section title="How payment works" Icon={CreditCard} accent={C.amber}>
          <p style={{ color: C.sub, fontSize: 13.5, marginTop: -4, marginBottom: 16 }}>
            Phase by phase, milestone based. You are only ever committed to the next piece in front of you, and you see every
            number before you say yes to it.
          </p>
          <div style={{ display: "grid", gap: 12, gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))" }}>
            {payMilestones.map((m, i) => (
              <div key={i} style={{ background: C.card, border: `1px solid ${i === 0 ? m.accent + "88" : C.border}`, borderRadius: 16, padding: 18, boxShadow: i === 0 ? `0 0 0 1px ${m.accent}22` : "none" }}>
                <div style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 10.5, fontWeight: 800, letterSpacing: 1, textTransform: "uppercase", color: m.accent, background: m.accent + "1A", padding: "4px 9px", borderRadius: 99 }}>
                  <m.Icon size={12} /> {m.tag}
                </div>
                <div style={{ fontSize: 15, fontWeight: 700, marginTop: 12 }}>{m.title}</div>
                <div style={{ fontSize: 22, fontWeight: 800, marginTop: 6, color: m.accent }}>{m.price}</div>
                <div style={{ fontSize: 12, color: C.muted, marginTop: 2 }}>{m.when}</div>
                <p style={{ fontSize: 13, color: C.sub, lineHeight: 1.55, margin: "10px 0 0" }}>{m.desc}</p>
              </div>
            ))}
          </div>
          <div style={{ marginTop: 14, background: C.bg2, border: `1px solid ${C.border}`, borderRadius: 12, padding: "12px 16px", display: "flex", gap: 10, alignItems: "flex-start" }}>
            <ShieldCheck size={16} style={{ color: C.green, flexShrink: 0, marginTop: 2 }} />
            <span style={{ fontSize: 13, color: C.sub, lineHeight: 1.55 }}>
              No hidden charges, no surprise invoices. The software the systems run on (GoHighLevel, Notion and the like) you
              own and pay directly, and I flag every one before we switch it on.
            </span>
          </div>
        </Section>
        ...
        <div style={{ marginTop: 44, paddingTop: 24, borderTop: `1px solid ${C.border}`, display: "flex", flexWrap: "wrap", gap: 16, alignItems: "center", justifyContent: "space-between" }}>
          ...
          <div style={{ display: "flex", gap: 18, flexWrap: "wrap" }}>
            <a href="mailto:bvn@bvnofficial.com" style={{ display: "inline-flex", alignItems: "center", gap: 7, color: C.sub, fontSize: 13, textDecoration: "none" }}>
              <Mail size={15} /> bvn@bvnofficial.com
            </a>
