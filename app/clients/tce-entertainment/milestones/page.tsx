...
const PORTALS: Portal[] = [
  {
    ...
    bullets: [
      "Event questionnaire: ceremony, cocktail, reception, first dance, parent dances, intros, timeline, name pronunciation, vendor contacts",
      "Must-play and do-not-play lists + special announcements",
      "Song library + AI song / playlist suggestions, then final music selections",
      "Contract, deposit + final invoices, COI, and all documents in one place",
    ],
  },
  {
    Icon: Music, name: "Talent portal", sub: "GHL — Talent sub-account", color: C.cyan,
    bullets: [
      "Gig offers to accept / decline; event info: pay, role, venue, call time, dress code, parking, lineup, emergency contact",
      "Stage plot, set list (song, key, vocalist, charts, backing tracks, audio) + Google Drive resources",
      "Upcoming + past gigs, calendar (auto-added on accept), production schedule",
      "Payments, contracts, and tax documents (W-9, direct deposit)",
    ],
  },
  {
    Icon: LayoutDashboard, name: "Admin command center", sub: "Across both sub-accounts", color: C.amber,
    bullets: [
      "Receives everything across Sales, Finance, Production, and Booking",
      "Talent database search + AI nearest-band matching to assemble the band",
      "Contracts, deposit / 21-day final reminders, and music-director packet",
      "Payout list: per-artist rate, gated to completed gigs, paid manually",
    ],
  },
];
...
const phases: Phase[] = [
  {
    ...
    objective: "Lock the data model and access so nothing is rebuilt later. Map the existing SOP onto the two sub-accounts and confirm the talent database, tags, and rates the matching engine runs on.",
    items: [
      { label: "Access to both GHL sub-accounts (or create Client + Talent sub-accounts)", status: "todo" },
      { label: "Import / confirm talent database: name, instrument, role, city/state, travel radius, rate, availability", status: "todo" },
      ...
      { label: "Pull branding + song library structure from troycurtisentertainment.com and Google Drive", status: "todo" },
    ],
  },
  {
    code: "Phase 1", name: "Client side — Sales, Finance, contract", window: "Weeks 1–2",
    Icon: Handshake, accent: C.coral,
    objective: "SOP Phases 1–2. Package selection to signed contract, then the finance automation: deposit invoice, collection, and the 21-day final-payment reminder.",
    items: [
  ...
  {
    code: "Phase 2", name: "Client portal — music planning", window: "Weeks 2–3",
    Icon: Music, accent: C.purple,
    objective: "SOP Phases 3 & 5. Production onboarding docs plus the client portal: event questionnaire, song library, and final music selections.",
    items: [
      { label: "Production onboarding: welcome, contact, stage/technical/hospitality riders, electrical + sound, timeline", status: "todo" },
      { label: "Client portal event questionnaire (ceremony → reception, must-play / do-not-play, timeline, vendor contacts)", status: "todo" },
      { label: "Song library + client music selections capture", status: "todo" },
      { label: "Venue coordination + COI request/tracking (Phase 4)", status: "todo" },
    ],
  },
  {
    code: "Phase 3", name: "Talent side — booking, offers, portal", window: "Weeks 3–4",
    Icon: Users, accent: C.cyan,
    objective: "SOP Phases 6–8. Talent database search, gig offers with accept/decline, calendar + reminders, and the full talent portal.",
    items: [
      ...
      { label: "Talent portal: upcoming/past gigs, calendar, payments, contracts, tax docs, charts, set lists", status: "todo" },
    ],
  },
  {
  ...
  {
    code: "Phase 5", name: "Command center, post-event, launch", window: "Weeks 5–6",
    Icon: LayoutDashboard, accent: C.amber,
    objective: "SOP Phases 10–13. Admin command center, post-event recap, completed-gig payouts, client follow-up, and launch on the TCE website with the three portals.",
    items: [
      { label: "Admin command center: unified view + assign band + generate proposal/contract", status: "todo" },
...
const OPEN: { q: string; why: string }[] = [
  ...
  { q: "Where do the performance resources live — one Google Drive we standardize, or scattered?", why: "The talent portal links set lists, charts, and backing tracks straight from Drive (SOP Step 5)." },
  ...
  { q: "Should the AI song helper draw from your existing song library, or suggest freely?", why: "You chose open LLM — confirming it can still lean on the library where it exists." },
      ...
      <div style={{ maxWidth: 1000, margin: "0 auto", padding: "28px 20px 90px" }}>
        ...
        <div style={{ marginTop: 22 }}>
          ...
          <p style={{ color: C.sub, fontSize: 15, maxWidth: 720, margin: 0, lineHeight: 1.6 }}>
            Built directly from your Production SOP and Artist Platform SOP. Two GoHighLevel sub-accounts, three portals, an
            AI matching and song layer, and one admin command center — mapped step for step to your existing 13-phase journey
            across Sales, Finance, Production, and Booking. This is the shared source of truth, updated as each item moves.
          </p>
        </div>
