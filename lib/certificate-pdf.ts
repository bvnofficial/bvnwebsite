// Shared certificate PDF builder. Used by both the download route
// (/api/certificate/[id]/pdf) and the email helper (so the emailed certificate
// is byte-for-byte identical to the downloadable one). Server-only.
...
export async function buildCertificatePdf(row: CompletionRow): Promise<Uint8Array> {
